...
int match(shared_ptr<AnimalCard> around, shared_ptr<AnimalCard> card, int around_coord, int card_coord) {
	return around->getAnimalAt(around_coord) == card->getAnimalAt(card_coord) /*match of same animal in both cards*/
...
void Table::printTable() {
	...
	ainsi que la derniere ligne et derniere colonne*/
	for (int l = 0; l < 103; l++) {
		for (int c = 0; c < 103; c++) {
			if (table[l][c] != 0) {
				if (l < startRow)
	...
	/*on imprime aussi les coordonnees pour aider l'utilisateur*/
